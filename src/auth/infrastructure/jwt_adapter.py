...
class JwtAdapter(IJwtAdapter):
    ...
    def sign(self, payload: TokenPayload) -> str:
        ...
        return jwt.encode(
            data, self._config.jwt_access_token_secret, algorithm=self._config.jwt_algo
        )

    def verify(self, token: str) -> TokenPayload:
        data = jwt.decode(
            token, self._config.jwt_access_token_secret, algorithms=[self._config.jwt_algo]
        )
    ...
    def sign_refresh(self, payload: TokenPayload) -> str:
        ...
        return jwt.encode(
            data, self._config.jwt_refresh_token_secret, algorithm=self._config.jwt_algo
        )

    def verify_refresh(self, token: str) -> TokenPayload | None:
        data = jwt.decode(
            token, self._config.jwt_refresh_token_secret, algorithms=[self._config.jwt_algo]
        )
        return TokenPayload(sub=data["sub"], email=data["email"], role=data["role"])
